...
class MetricEvent:
    ...
    def __post_init__(self):
        """Initialize metric ID if not provided."""
        if self.id is None:
...
class UnifiedMetricsCollector:
    ...
    def __init__(self, root: Path):
        ...
        self.collection_stats: Dict[str, Union[int, float, str, None]] = {
            "total_collected": 0,
            "collection_errors": 0,
        ...
        self.collection_stats["total_collected"] = int(
            self.collection_stats["total_collected"] or 0
        )
        self.collection_stats["collection_errors"] = int(
            self.collection_stats["collection_errors"] or 0
        )
        self.collection_stats["avg_collection_time_ms"] = float(
            self.collection_stats["avg_collection_time_ms"] or 0.0
        )
    ...
    def batch_collect(self, metrics: List[MetricEvent]) -> List[str]:
        ...
        if not metrics or not self.config.get("collection_enabled", True):
            return [m.id or "" for m in metrics]
        ...
        try:
            ...
            for metric in metrics:
                ...
                collected_ids.append(metric.id or "")
    ...
    def _update_collection_stats(self, collection_time_ms: float, success: bool):
        ...
        if success:
            # Update counters (values are guaranteed to be int/float by initialization)
            total_collected = int(self.collection_stats["total_collected"]) + 1  # type: ignore
            self.collection_stats["total_collected"] = total_collected

            # Update rolling average
            current_avg = float(self.collection_stats["avg_collection_time_ms"])  # type: ignore
            total = total_collected
            new_avg = ((current_avg * (total - 1)) + collection_time_ms) / total
            self.collection_stats["avg_collection_time_ms"] = new_avg
        else:
            collection_errors = int(self.collection_stats["collection_errors"]) + 1  # type: ignore
            self.collection_stats["collection_errors"] = collection_errors
