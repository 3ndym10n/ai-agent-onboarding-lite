...
class FileOrganizationAnalyzer:
    ...
    def _analyze_file_complexity(self, file_path: str) -> None:
        ...
        try:
            ...
            self.file_complexity[file_path] = 0.0
    ...
    def _analyze_file_relationships(self) -> None:
        ...
        for file_path in self.all_files:
            if processed_count >= self.max_relationship_files:
                print(
                    (
                        f"ℹ️ Limiting relationship analysis to {self.max_relationship_files} Python files "
                        "(configurable via max_relationship_files parameter)"
                    )
                )
                break
