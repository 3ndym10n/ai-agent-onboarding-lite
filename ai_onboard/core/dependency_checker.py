...
class DependencyChecker:
    ...
    def check_dependencies(self, target_files: List[Path]) -> List[DependencyCheckResult]:
        """
        ...
        print_content(f"Checking dependencies for {len(target_files)} files...", "search")
        ...
        for target_file in target_files:
            ...
            if not result.is_safe_to_delete:
                print_status(f"⚠️ {target_file.name} has {len(result.dependencies)} dependencies", "warning")
        
        return results

    def _check_single_file_dependencies(self, target_file: Path) -> DependencyCheckResult:
        ...
        for pattern in self.scannable_patterns:
            for source_file in self.root.glob(pattern):
                ...
                file_dependencies = self._scan_file_for_references(source_file, target_file)
        ...
        if not is_safe:
            recommendations.append(f"Update {len(dependencies)} dependencies before deletion")
    ...
    def _scan_file_for_references(self, source_file: Path, target_file: Path) -> List[Dependency]:
        ...
        try:
            ...
            if source_file.suffix == '.py':
                dependencies.extend(self._scan_python_file(source_file, target_file, content))
            elif source_file.suffix in ['.json', '.yaml', '.yml', '.toml']:
                dependencies.extend(self._scan_config_file(source_file, target_file, content))
            elif source_file.suffix in ['.md', '.rst', '.txt']:
                dependencies.extend(self._scan_documentation_file(source_file, target_file, content))
            else:
                dependencies.extend(self._scan_generic_file(source_file, target_file, content))
    ...
    def _scan_python_file(self, source_file: Path, target_file: Path, content: str) -> List[Dependency]:
        ...
        target_patterns = [
            str(target_file),
            str(target_file.relative_to(self.root)) if target_file.is_relative_to(self.root) else None,
            target_file.name,
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern in target_patterns:
                if pattern and pattern in line:
                    ...
                    elif any(keyword in line.lower() for keyword in ['read', 'open', 'path', 'file']):
                    ...
                    dependencies.append(Dependency(
                        ...
                        context=line.strip(),
                        severity="high" if dep_type == DependencyType.PYTHON_IMPORT else "medium"
                    ))
    ...
    def _scan_config_file(self, source_file: Path, target_file: Path, content: str) -> List[Dependency]:
    ...
    def _scan_documentation_file(self, source_file: Path, target_file: Path, content: str) -> List[Dependency]:
    ...
    def _scan_generic_file(self, source_file: Path, target_file: Path, content: str) -> List[Dependency]:
    ...
    def _matches_target(self, module_name: str, target_file: Path) -> bool:
        ...
        if target_file.suffix == '.py':
            ...
            relative_path = target_file.relative_to(self.root) if target_file.is_relative_to(self.root) else target_file
            module_path = str(relative_path.with_suffix('')).replace('/', '.').replace('\\', '.')
            
            return module_name == module_path or module_name.endswith('.' + target_file.stem)
        
        return False

    def _assess_risk(self, dependencies: List[Dependency], target_file: Path) -> Tuple[str, bool]:
                ...
                if not result.is_safe_to_delete:
                    ...
                    if result.dependencies:
                        ...
                        for dep in result.dependencies[:5]:  # Show first 5
                            report.append(f"     - {dep.source_file.name}:{dep.line_number} ({dep.dependency_type.value})")
                        
                        if len(result.dependencies) > 5:
                            report.append(f"     ... and {len(result.dependencies) - 5} more")
    ...
    def create_dependency_fix_plan(self, results: List[DependencyCheckResult]) -> Dict[str, Any]:
...
def check_cleanup_dependencies(root: Path, target_files: List[Path]) -> bool:
    ...
    if unsafe_files:
        print_status(f"❌ {len(unsafe_files)} files have dependencies and cannot be safely deleted", "error")
        ...
        safe_print(f"   Manual review required: {len(fix_plan['manual_review_needed'])}")
