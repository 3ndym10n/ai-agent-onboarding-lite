...
class KnowledgeBaseEvolution:
    ...
    def __init__(self, root: Path):
        ...
        self.recommendations_path = root / ".ai_onboard" / "knowledge_recommendations.jsonl"
        self.evolution_config_path = root / ".ai_onboard" / "knowledge_evolution_config.json"
        ...
        self.knowledge_graph: Dict[str, Set[str]] = defaultdict(set)  # knowledge_id -> related_ids
    ...
    def _load_evolution_config(self) -> Dict[str, Any]:
        ...
        return utils.read_json(self.evolution_config_path, default={
    ...
    def _validate_knowledge_quality(self, knowledge_item: KnowledgeItem) -> bool:
        ...
        if knowledge_item.confidence_score < self.evolution_config["knowledge_quality_threshold"]:
            return False
        
        # Check for required fields based on knowledge type
        if knowledge_item.knowledge_type == KnowledgeType.ERROR_SOLUTION:
            if not any(keyword in knowledge_item.content.lower() for keyword in ["solution", "fix", "resolve", "error"]):
                return False
    ...
    def _calculate_query_relevance(self, query: str, knowledge_item: KnowledgeItem) -> float:
        ...
        content_relevance = len(query_words.intersection(content_words)) / len(query_words) if query_words else 0
    ...
    def _find_contextual_knowledge(self, context: Dict[str, Any]) -> List[KnowledgeItem]:
    ...
    def _calculate_context_relevance(
        ...
        # Check content relevance
        content_words = set(knowledge_item.content.lower().split())
        content_relevance = len(context_words.intersection(content_words)) / len(context_words) if context_words else 0
        ...
        for error_type, items in error_groups.items():
            if len(items) >= self.evolution_config["pattern_min_frequency"]:
                pattern = KnowledgePattern(
                    ...
                    outcomes={"resolution_time": "reduced", "success_rate": "increased"},
                    discovered_at=datetime.now(),
                    last_updated=datetime.now()
                )
                patterns.append(pattern)
    ...
    def _item_matches_pattern(self, item: KnowledgeItem, pattern: KnowledgePattern) -> bool:
    ...
    def _find_knowledge_conflicts(self) -> List[List[KnowledgeItem]]:
        ...
        for item1 in self.knowledge_items.values():
            ...
            for item2 in self.knowledge_items.values():
                if item2.knowledge_id in processed or item1.knowledge_id == item2.knowledge_id:
                    continue
    ...
    def _is_knowledge_conflict(self, item1: KnowledgeItem, item2: KnowledgeItem) -> bool:
    ...
    def _adapt_knowledge_relevance(self):
        ...
        for item in self.knowledge_items.values():
            ...
            if item.last_accessed:
                ...
                access_factor = max(0, 1 - (days_since_access / 30))  # Decay over 30 days
            else:
                access_factor = 0
    ...
    def get_knowledge_statistics(self) -> Dict[str, Any]:
        ...
        active_items = len([item for item in self.knowledge_items.values() if item.status == KnowledgeStatus.ACTIVE])
        
        # Knowledge type distribution
        type_distribution = Counter(item.knowledge_type.value for item in self.knowledge_items.values())
        
        # Quality distribution
        quality_distribution = Counter(item.quality.value for item in self.knowledge_items.values())
        
        # Source distribution
        source_distribution = Counter(item.source.value for item in self.knowledge_items.values())
        
        # Average confidence and relevance
        avg_confidence = statistics.mean([item.confidence_score for item in self.knowledge_items.values()]) if self.knowledge_items else 0
        avg_relevance = statistics.mean([item.relevance_score for item in self.knowledge_items.values()]) if self.knowledge_items else 0
        
        # Pattern statistics
        total_patterns = len(self.knowledge_patterns)
        avg_pattern_confidence = statistics.mean([p.confidence for p in self.knowledge_patterns.values()]) if self.knowledge_patterns else 0
        
        # Recommendation statistics
        total_recommendations = len(self.knowledge_recommendations)
        active_recommendations = len([r for r in self.knowledge_recommendations if not r.expires_at or r.expires_at > datetime.now()])
    ...
    def _save_knowledge_item(self, knowledge_item: KnowledgeItem):
        ...
        data = {
            ...
            "last_accessed": knowledge_item.last_accessed.isoformat() if knowledge_item.last_accessed else None,
