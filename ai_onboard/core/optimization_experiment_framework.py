...
from .unified_metrics_collector import get_unified_metrics_collector, MetricEvent, MetricSource, MetricCategory
...
class RunningExperiment:
    ...
    # Runtime state
    samples_collected: Dict[str, int] = field(default_factory=dict)  # condition_id -> count
...
class StatisticalAnalyzer:
    ...
    def analyze_ab_test(self, control_values: List[float], treatment_values: List[float]) -> Dict[str, Any]:
        ...
        ci_lower = improvement - (margin_of_error / control_mean * 100) if control_mean != 0 else 0
        ci_upper = improvement + (margin_of_error / control_mean * 100) if control_mean != 0 else 0
        ...
        return {
            ...
            "recommendation": self._generate_recommendation(improvement, significance, p_value)
        }
    
    def analyze_multivariate(self, condition_data: Dict[str, List[float]]) -> Dict[str, Any]:
        ...
        improvement = ((best_condition[1] - baseline_condition[1]) / baseline_condition[1]) * 100 if baseline_condition[1] != 0 else 0
    ...
    def _generate_recommendation(self, improvement: float, significance: StatisticalSignificance, p_value: float) -> str:
...
class OptimizationExperimentFramework:
    ...
    def __init__(self, root: Path):
        ...
        self._executor = ThreadPoolExecutor(max_workers=3, thread_name_prefix="experiment")
            ...
            if designs_file.exists():
                with open(designs_file, 'r') as f:
                    ...
                    for exp_id, design_data in data.items():
                        ...
                        design_data['experiment_type'] = ExperimentType(design_data['experiment_type'])
                        design_data['created_at'] = datetime.fromisoformat(design_data['created_at'])
                        ...
                        self.experiment_designs[exp_id] = ExperimentDesign(**design_data)
            ...
            if results_file.exists():
                with open(results_file, 'r') as f:
                    ...
                    for exp_id, result_data in data.items():
                        result_data['outcome'] = ExperimentOutcome(result_data['outcome'])
                        result_data['significance'] = StatisticalSignificance(result_data['significance'])
                        self.experiment_results[exp_id] = ExperimentResults(**result_data)
    ...
    def stop_experiment(self, experiment_id: str, reason: str = "manual_stop") -> bool:
        ...
        print(f"Outcome: {results.outcome.value}, Improvement: {results.improvement_percentage:.2f}%")
        
        return True
    
    def record_measurement(self, experiment_id: str, condition_id: str, metric_name: str, value: float, context: Optional[Dict[str, Any]] = None) -> bool:
        ...
        running_exp.samples_collected[condition_id] = running_exp.samples_collected.get(condition_id, 0) + 1
    ...
    def _analyze_experiment_results(self, running_exp: RunningExperiment) -> ExperimentResults:
        ...
        for measurement in running_exp.measurements:
            ...
            condition_data[measurement.condition_id][measurement.metric_name].append(measurement.value)
        ...
        if design.experiment_type == ExperimentType.AB_TEST and len(primary_metric_data) == 2:
            ...
            statistical_analysis = self.statistical_analyzer.analyze_ab_test(control_data, treatment_data)
        else:
            statistical_analysis = self.statistical_analyzer.analyze_multivariate(primary_metric_data)
        ...
        significance = statistical_analysis.get('significance', StatisticalSignificance.NOT_SIGNIFICANT)
        confidence_interval = statistical_analysis.get('confidence_interval', (0.0, 0.0))
        
        # Generate recommendations
        recommendation = statistical_analysis.get('recommendation', 'No specific recommendation')
        lessons_learned = self._extract_lessons_learned(running_exp, statistical_analysis)
        next_experiments = self._suggest_next_experiments(running_exp, statistical_analysis)
    ...
    def _determine_experiment_outcome(self, analysis: Dict[str, Any]) -> ExperimentOutcome:
        """Determine the overall outcome of an experiment."""
        improvement = analysis.get('improvement_percentage', 0)
        significance = analysis.get('significance', StatisticalSignificance.NOT_SIGNIFICANT)
    ...
    def _extract_lessons_learned(self, running_exp: RunningExperiment, analysis: Dict[str, Any]) -> List[str]:
        ...
        if significance != StatisticalSignificance.NOT_SIGNIFICANT:
            if improvement > 0:
                lessons.append(f"Optimization achieved {improvement:.1f}% improvement with {significance.value} significance")
            else:
                lessons.append(f"Optimization caused {abs(improvement):.1f}% regression with {significance.value} significance")
        else:
            lessons.append("No statistically significant difference detected - may need larger sample size or longer runtime")
        
        # Sample size lessons
        total_samples = sum(running_exp.samples_collected.values())
        if total_samples < running_exp.design.sample_size_per_condition * len(running_exp.design.conditions):
            lessons.append("Experiment ended before reaching target sample size - consider longer runtime")
        
        return lessons
    
    def _suggest_next_experiments(self, running_exp: RunningExperiment, analysis: Dict[str, Any]) -> List[str]:
        ...
        if outcome == ExperimentOutcome.SUCCESS:
            suggestions.append("Consider testing more aggressive optimization parameters")
            suggestions.append("Test the optimization on different workloads or conditions")
    ...
    def _check_early_stopping(self, running_exp: RunningExperiment):
        ...
        min_samples = max(30, running_exp.design.sample_size_per_condition // 4)  # At least 25% of target
        ...
        if len(primary_metric_data) >= 2:
            if running_exp.design.experiment_type == ExperimentType.AB_TEST:
                ...
                interim_analysis = self.statistical_analyzer.analyze_ab_test(control_data, treatment_data)
                ...
                if improvement < -running_exp.design.rollback_threshold * 100:
                    ...
                    print(f"Early stopping triggered: {improvement:.1f}% regression detected")
                    self.stop_experiment(running_exp.design.experiment_id, "harmful_regression_detected")
        ...
        while self._monitoring_active:
            try:
                ...
                for exp_id, running_exp in self.running_experiments.items():
                    ...
                    runtime_hours = (current_time - running_exp.started_at).total_seconds() / 3600
                    ...
                    total_samples = sum(running_exp.samples_collected.values())
                    target_samples = running_exp.design.sample_size_per_condition * len(running_exp.design.conditions)
                    
                    if total_samples >= target_samples and runtime_hours >= running_exp.design.minimum_runtime_hours:
    ...
    def get_experiment_status(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        if experiment_id in self.running_experiments:
            running_exp = self.running_experiments[experiment_id]
            runtime_hours = (datetime.now() - running_exp.started_at).total_seconds() / 3600
    ...
    def get_all_experiments(self) -> Dict[str, Any]:
        ...
        return {
            "running": {exp_id: self.get_experiment_status(exp_id) for exp_id in self.running_experiments},
            "completed": {exp_id: self.get_experiment_status(exp_id) for exp_id in self.experiment_results},
            "designs": {exp_id: design.name for exp_id, design in self.experiment_designs.items()}
        }
...
def get_optimization_experiment_framework(root: Path) -> OptimizationExperimentFramework:
