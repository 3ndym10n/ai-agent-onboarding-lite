...
# alignment import moved to vision package
from ..vision.alignment import preview as alignment_preview
...
class AIAgentIASWrapper:
    ...
    def get_alignment_preview(self) -> Dict[str, Any]:
        ...
        try:
            return alignment_preview(self.root)
    ...
    def generate_conversation_prompt(self, alignment_data: Dict[str, Any]) -> str:
        ...
        if decision == "proceed":
            ...
            content = self._format_quick_confirm_content(alignment_data)
            return (
                f"🤔 Medium confidence ({confidence:.2f}) - I have a good understanding but want to confirm. "
                f"Here's what I think:\n\n{content}\n\nDoes this sound right to you?"
            )

        else:  # clarify
            clarify_content = self._format_clarify_content(alignment_data)
            return (
                f"❓ Low confidence ({confidence:.2f}) - I need some clarification before proceeding.\n"
                f"Here's what I found:\n\n{clarify_content}\n\n"
                f"Can you help me understand what I'm missing or correct my assumptions?"
            )
    ...
    def process_user_response(
        ...
        if self.conversation_rounds > self.guardrails.MAX_CONVERSATION_ROUNDS:
            return {
                ...
                "message": (
                    "Maximum conversation rounds reached. Please use --assume proceed "
                    "or address issues manually."
                ),
                "can_proceed": False,
            }
    ...
    def execute_command_with_flags(
        self, command: str, flags: Optional[List[str]] = None
    ) -> Dict[str, Any]:
