...
from ..orchestration.unified_tool_orchestrator import (
...
class IntelligentDevelopmentMonitor:
    ...
    def _rule_matches_activity(
        ...
        for condition_key, condition_value in rule.context_conditions.items():
            ...
            if condition_key == "file_types":
                ...
                if activity_value is None or activity_value <= threshold:
                    return False
    ...
    def _execute_trigger_rule(
        ...
        for tool_name in rule.required_tools:
            ...
            execution_context = ToolExecutionContext(
                user_request=f"Execute {tool_name}", additional_context=tool_context
            )
    ...
    def manually_trigger_analysis(
        self, analysis_type: str, context: Optional[Dict[str, Any]] = None
    ):
        ...
        if analysis_type == "code_quality":
            execution_context = ToolExecutionContext(
                user_request=f"Execute code quality analysis",
            ...
            execution_context = ToolExecutionContext(
                user_request=f"Execute organization analysis",
                additional_context=context,
            )
            result = self.tool_orchestrator.execute_automatic_tool_application(
                execution_context
            ...
            execution_context = ToolExecutionContext(
                user_request=f"Execute risk analysis", additional_context=context
            )
            result = self.tool_orchestrator.execute_automatic_tool_application(
                execution_context
            )
            ...
            for tool in tools:
                execution_context = ToolExecutionContext(
                    user_request=f"Execute {tool}", additional_context=context
                )
                result = self.tool_orchestrator.execute_automatic_tool_application(
                    execution_context
                )
