...
from ..legacy_cleanup import smart_debugger
from ..orchestration import universal_error_monitor
...
class ContinuousImprovementSystem:
    ...
    def _process_performance_learning(self, event: LearningEvent):
        ...
        op_type = event.context.get("operation_type", "unknown")
        proj_type = event.context.get("project_type", "generic")
        optimization_key = f"{op_type}_{proj_type}"
    ...
    def _generate_project_specific_recommendations(
        ...
        if success_indicators.get("vision_clarity", 0) > 0.8:
            recommendation = ImprovementRecommendation(
                ...
                rationale=(
                    f"High success rate ({success_indicators.get('vision_clarity', 0):.2f}) "
                    "suggests good patterns"
                ),
                expected_impact=0.3,
                confidence=0.8,
                priority=5,
