...
This module provides comprehensive analytics and \
    reporting capabilities for the continuous improvement system:
...
class ContinuousImprovementAnalytics:
                ...
                for level, threshold in thresholds.items():
                    if metric.value <= threshold:
                        self._create_alert(
                            ...
                            description=(
                                f"{kpi_def['name']} is {metric.value:.2f}, "
                                f"below {level} threshold of {threshold}"
                            ),
    ...
    def _update_related_kpis(self, metric: Metric) -> None:
    ...
    def _generate_report_summary(
        ...
        if report_type == ReportType.PERFORMANCE_SUMMARY:
            avg_value = data["summary"]["average_value"]
            total_metrics = data["summary"]["total_metrics"]
            return f"Performance summary shows {total_metrics} metrics tracked with average value of {avg_value:.2f}."
        elif report_type == ReportType.LEARNING_ANALYTICS:
            return "Learning analytics indicate stable knowledge acquisition patterns."
        elif report_type == ReportType.SYSTEM_HEALTH:
            trend = data["health_score"]["trend"]
            current_score = data["health_score"]["current"]
            return (
                f"System health is {trend} with current score of {current_score:.1f}."
            )
        else:
