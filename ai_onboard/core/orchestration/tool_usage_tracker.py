...
class ToolUsageTracker:
    ...
    def display_tools_summary(self, session_summary: Dict[str, Any]) -> None:
        ...
        if not session_summary or session_summary.get("total_tools_used", 0) == 0:
            ensure_unicode_safe("ℹ️  No system tools were used during this task.")
            return

        print("\n" + "=" * 60)
        ensure_unicode_safe("🔧 SYSTEM TOOLS USED DURING TASK EXECUTION")
        print("=" * 60)

        tools_summary = session_summary.get("tools_summary", {})

        # Overall stats
        ensure_unicode_safe("📊 OVERVIEW:")
...
def get_tool_tracker(root: Path) -> ToolUsageTracker:
    ...
    if _tool_tracker is None or _tool_tracker.root != root:  # type: ignore[unreachable]
