...
def _generate_charter_aware_wbs(charter: dict) -> list:
    ...
    if "agent" in str(charter.get("objectives", [])).lower():
        wbs.append({"id": "C4", "name": "AI agent collaboration features", "deps": ["C1", "C2"]})
    
    # Continuous improvement
    if "improvement" in str(charter.get("objectives", [])).lower():
        wbs.append({"id": "C5", "name": "Continuous improvement system", "deps": ["C1", "C2"]})
    ...
    wbs.append({"id": "C7", "name": "Documentation and user experience", "deps": ["C1", "C2", "C3"]})
    
    # System robustness and self-improvement
    wbs.append({"id": "C8", "name": "System robustness and self-improvement", "deps": ["C1", "C2", "C4"]})
...
def _generate_charter_aware_milestones(charter: dict) -> list:
    ...
    milestones = [
        {"name": "Vision Confirmation", "description": "Project vision and objectives defined"},
        {"name": "Core System Ready", "description": "Basic system functionality implemented"},
        {"name": "AI Agent Integration", "description": "AI agent collaboration features working"},
    ...
    if any("alignment" in outcome.lower() for outcome in top_outcomes):
        milestones.append({"name": "Alignment System Complete", "description": "AI agent alignment features fully implemented"})
...
def _generate_detailed_tasks(charter: dict, wbs: list) -> list:
    ...
    wbs_task_map = {
        "C1": [
            ...
            {"name": "Configure development environment", "effort_days": 1, "type": "setup"},
            {"name": "Set up version control and CI/CD", "effort_days": 1, "type": "setup"}
        ],
        "C2": [
            {"name": "Design vision interrogation system", "effort_days": 3, "type": "design"},
            {"name": "Implement vision clarity scoring", "effort_days": 2, "type": "development"},
            {"name": "Create vision validation logic", "effort_days": 2, "type": "development"}
        ],
        "C3": [
            {"name": "Enhance gate system timeout handling", "effort_days": 1, "type": "development"},
            {"name": "Improve gate response processing", "effort_days": 2, "type": "development"},
            {"name": "Add adaptive gate triggering", "effort_days": 3, "type": "development"}
        ],
        "C4": [
            {"name": "Design AI agent collaboration protocol", "effort_days": 2, "type": "design"},
            {"name": "Implement conversation context management", "effort_days": 3, "type": "development"},
            {"name": "Create agent decision pipeline", "effort_days": 4, "type": "development"},
            {"name": "Build user preference learning system", "effort_days": 5, "type": "development"}
        ],
        "C5": [
            {"name": "Design metrics collection system", "effort_days": 2, "type": "design"},
            {"name": "Implement Kaizen cycle automation", "effort_days": 3, "type": "development"},
            {"name": "Create optimization experiment framework", "effort_days": 3, "type": "development"}
        ],
        "C6": [
            {"name": "Research Cursor AI integration points", "effort_days": 2, "type": "research"},
            {"name": "Implement Cursor AI API integration", "effort_days": 4, "type": "development"},
            {"name": "Test integration with real Cursor workflows", "effort_days": 3, "type": "testing"}
        ],
        "C7": [
            {"name": "Create user documentation", "effort_days": 3, "type": "documentation"},
            {"name": "Design user interface improvements", "effort_days": 2, "type": "design"},
            {"name": "Implement user experience enhancements", "effort_days": 3, "type": "development"}
        ],
        "C8": [
            {"name": "Implement automatic error interception", "effort_days": 2, "type": "development"},
            {"name": "Create system capability usage tracking", "effort_days": 2, "type": "development"},
            {"name": "Build learning feedback loops", "effort_days": 3, "type": "development"},
            {"name": "Design background agent integration", "effort_days": 4, "type": "development"}
        ]
    }
        ...
        if wbs_id in wbs_task_map:
            for task_detail in wbs_task_map[wbs_id]:
                task = {
                    ...
                    "status": "pending",
                    "dependencies": [],
                    "assigned_to": "dev" if task_detail["type"] in ["development", "testing"] else "owner"
                }
                tasks.append(task)
                task_id += 1
        ...
        if task_type in type_dependencies:
            for dep_type in type_dependencies[task_type]:
                ...
                for other_task in tasks:
                    if (other_task["type"] == dep_type and 
                        other_task["id"] != task["id"] and
                        other_task["wbs_id"] <= task["wbs_id"]):
                        dependencies.append({
                            "from": other_task["id"],
                            "to": task["id"],
                            "type": "logical"
