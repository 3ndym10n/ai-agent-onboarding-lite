...
class MandatoryToolConsultationGate:
    ...
    def __init__(self, root_path: Path):
        ...
        self.gate_config = {
            ...
            "require_high_relevance_tools": False,  # Allow passage without high-relevance tools
            "allow_bypass": False,
    ...
    def _extract_tool_insights(self, tool_name: str, results: Any) -> Optional[str]:
        ...
        try:
            ...
            elif tool_name == "file_organization_analysis":
                if hasattr(results, "issues") and hasattr(results, "files_analyzed"):
                    return f"{len(results.issues)} organization issues in {results.files_analyzed} files"

            elif tool_name == "risk_assessment":
                if hasattr(results, "high_risk_changes"):
                    high_risk = len(
                        [c for c in results.changes if c.risk_level.value == "high"]
                    )
                    return f"{high_risk} high-risk changes identified"

            elif tool_name == "duplicate_detection":
                if hasattr(results, "duplicate_groups"):
                    return (
                        f"{len(results.duplicate_groups)} duplicate code groups found"
                    )
...
def enforce_tool_consultation(
    user_request: str, context: Dict[str, Any] = None, root_path: Path = None
) -> ToolConsultationResult:
    """
