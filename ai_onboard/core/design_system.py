...
class DesignSystemManager:
        ...
        if self.components_file.exists():
            try:
                with open(self.components_file, "r") as f:
                    data = json.load(f)
                    return {
                        name: DesignComponent(**comp_data)
        ...
        if self.patterns_file.exists():
            try:
                with open(self.patterns_file, "r") as f:
                    data = json.load(f)
                    return {
                        name: DesignPattern(**pattern_data)
    ...
    def validate_design_consistency(self, design_description: str) -> Dict:
        ...
        if not used_tokens:
            suggestions.append(
                "Consider using established design tokens for consistency"
            )
    ...
    def get_design_system_summary(self) -> Dict:
        ...
        return {
            ...
            "token_categories": list(
                set(token.category for token in self.tokens.values())
            ),
            "component_categories": list(
