...
def create_progress_bar(percentage: float, width: int = 20) -> str:
    ...
    from ..base import progress_utils
...
class GateSystem:
    ...
    def create_gate(self, gate_request: GateRequest) -> Dict[str, Any]:
        ...
        print("[CLIPBOARD] AI Agent: Read the gate file and follow the instructions")
...
"""
            ...
            if self.response_file.exists():
                try:
                    ...
                    if self._validate_response(response):
                        if (
                            require_proceed
                            and response.get("user_decision") != "proceed"
                        ):
                            print(
                                "[WARNING] Confirmation did not approve proceed. "
                                "Stopping for safety."
                            )
                            return {
                        ...
                        print("[OK] Received user response via AI agent")
                        return (
                            response
                        ...
                        print("[X] Invalid response format, waiting...")
            ...
            if isinstance(response_text, str):
                ...
                for pattern in fake_patterns:
                    if pattern in response_lower:
                        print(
                            f"[WARNING] Detected potential fake response: "
                            f"'{response_text}'"
                        )
                        print(
    ...
    def _generate_confirmation_prompt(
        ...
        lines = [
            ...
            "",
            "[WARNING] The AI agent has proposed the following answers. "
        ...
        lines.extend(
            [
                "",
                "### What you need to do:",
                "- If these are correct, reply with decision 'proceed'.",
                "- If anything is wrong, reply with decision 'modify' "
                "and updated answers.",
                ...
                "}",
                "",
                "[INFO] The system will ONLY proceed if decision == 'proceed'.",
            ]
        )
        return "\n".join(lines)
    ...
    def _finalize_gate_files(self) -> None:
        ...
        try:
            ...
            while time.time() < deadline:
                ok = True
                if (
                    not self.current_gate_file.exists()
                    or self.current_gate_file.stat().st_size == 0
                ):
                    ok = False
                if (
                    not self.status_file.exists()
                    or self.status_file.stat().st_size == 0
                ):
                    ok = False
                if ok:
                    break
    ...
    def _cursor_rules_contract(self, code_hint: Optional[str]) -> str:
        ...
        if code_hint:
            header.append("")
            header.append(
                f"Active confirmation code (copy exactly after 'CONFIRM:'): {code_hint}"
            )
        return "\n".join(header) + "\n"
...
def create_clarification_gate(
    ...
    if confidence < 0.5:
        questions.append(
            "Should I proceed with the current plan, or would you like to modify it?"
        )
...
def create_scope_check_gate(
    project_root: Path, current_plan: List[str], context: Dict[str, Any]
) -> Dict[str, Any]:
...
def create_confirmation_gate(
    project_root: Path, action: str, context: Dict[str, Any]
) -> Dict[str, Any]:
