...
from . import utils, charter, planning, vision_guardian
...
class ContextContinuityManager:
    ...
    def resolve_context_drift(self, drift_type: str, resolution_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def validate_agent_decision(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        context_validation = self._validate_context_consistency(decision, current_context)
        
        # Combine validations
        overall_score = (vision_validation.get("alignment_score", 0) + 
                        context_validation.get("consistency_score", 0)) / 2
        ...
        return {
            ...
            "recommendation": "approve" if overall_score > 0.7 else "review" if overall_score > 0.3 else "reject"
        }
    
    def _rebuild_context(self) -> Dict[str, Any]:
        """Rebuild context from current project state."""
        # Get charter and plan
        charter_data = utils.read_json(self.root / ".ai_onboard" / "charter.json", default={})
    ...
    def _extract_key_decisions(self, decisions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _log_session(self, session_data: Dict[str, Any], session_summary: Dict[str, Any]) -> None:
    ...
    def _analyze_activity_patterns(self, sessions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        priority_shifts = min(variance / (avg_activities ** 2), 1.0) if avg_activities > 0 else 0.0
        
        return {
            "scope_expansion": scope_expansion,
            "priority_shifts": priority_shifts,
            "activity_trend": "increasing" if scope_expansion > 0.3 else "stable" if scope_expansion < 0.1 else "variable"
        }
    ...
    def _generate_drift_recommendations(self, drift_types: List[str]) -> List[str]:
        ...
        for drift_type in drift_types:
            if drift_type == "decision_drift":
                recommendations.append("Review recent decisions for consistency with project vision")
            elif drift_type == "scope_drift":
                recommendations.append("Reassess project scope and objectives")
            elif drift_type == "priority_drift":
                recommendations.append("Revisit project priorities and milestone sequencing")
        
        return recommendations
    
    def _validate_context_consistency(self, decision: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if decision_phase and current_phase and decision_phase != current_phase:
            ...
            issues.append(f"Decision phase '{decision_phase}' doesn't match current phase '{current_phase}'")
        ...
        if recent_decisions:
            similar_decisions = [
                ...
                if d.get("type") == decision.get("type")
            ]
        ...
        return {
            ...
            "issues": issues,
            "recommendation": "consistent" if consistency_score > 0.7 else "review" if consistency_score > 0.4 else "inconsistent"
        }
    ...
    def _get_active_milestones(self, plan_data: Dict[str, Any]) -> List[str]:
        ...
        return [m.get("name", "") for m in milestones if m.get("name", "") not in completed]
    ...
    def _resolve_vision_drift(self, context: Dict[str, Any], resolution_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _resolve_scope_drift(self, context: Dict[str, Any], resolution_data: Dict[str, Any]) -> Dict[str, Any]:
        """Resolve scope drift."""
    ...
    def _resolve_priority_drift(self, context: Dict[str, Any], resolution_data: Dict[str, Any]) -> Dict[str, Any]:
