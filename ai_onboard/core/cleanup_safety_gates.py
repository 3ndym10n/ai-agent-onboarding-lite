...
from .unicode_utils import safe_print, print_status, print_content, print_activity
from . import utils
        ...
        for target in operation.targets:
            ...
            for parent in target.parents:
                if parent.name in self.protected_directories:
                    message = f"Target is within protected directory {parent.name}: {target}"
                    self.log(context, f"FAIL: {message}")
                    return GateResult.FAIL, message
...
class DependencyAnalysisGate(SafetyGate):
    ...
    def validate(self, context: SafetyGateContext) -> Tuple[GateResult, str]:
        ...
        self.log(context, f"Analyzing dependencies for {len(operation.targets)} targets")
        ...
        if unsafe_files:
            message = f"Found {len(unsafe_files)} files with dependencies requiring review"
...
class RiskAssessmentGate(SafetyGate):
    ...
    def validate(self, context: SafetyGateContext) -> Tuple[GateResult, str]:
        ...
        if dependency_report:
            ...
            high_risk_deps = sum(1 for result in dependency_report 
            ...
            if total_deps > 0:
                risk_factors.append(f"Has {total_deps} dependencies ({high_risk_deps} high-risk)")
        ...
        for target in operation.targets:
            if any(pattern in target.name.lower() for pattern in 
                   ['config', 'setup', 'init', 'main', 'core']):
                risk_score += 25
        ...
        self.log(context, f"Risk level: {risk_assessment.level.value} (score: {risk_score})")
        ...
        if risk_assessment.level == RiskLevel.CRITICAL:
            message = f"CRITICAL risk level requires manual review (score: {risk_score})"
...
class HumanConfirmationGate(SafetyGate):
    ...
    def validate(self, context: SafetyGateContext) -> Tuple[GateResult, str]:
        ...
        report = self._generate_confirmation_report(operation, risk_assessment, dependency_report)
        ...
        try:
            ...
            expected = f"CONFIRM: {confirmation_code}" if confirmation_code != "CONFIRM" else "CONFIRM"
    ...
    def _generate_confirmation_report(self, operation: CleanupOperation, 
                                    risk_assessment: RiskAssessment,
                                    dependency_report: Optional[List[DependencyCheckResult]]) -> str:
        ...
        if dependency_report:
            ...
            if unsafe_files:
                ...
                for result in unsafe_files[:3]:
                    lines.append(f"     - {result.target_file.name}: {len(result.dependencies)} dependencies")
...
class BackupExecuteGate(SafetyGate):
    ...
    def validate(self, context: SafetyGateContext) -> Tuple[GateResult, str]:
        ...
        try:
            ...
            return GateResult.PASS, f"Operation completed successfully (backup: {backup_id})"
...
class PostOperationGate(SafetyGate):
    ...
    def validate(self, context: SafetyGateContext) -> Tuple[GateResult, str]:
        ...
        try:
            ...
            import ai_onboard
            validation_results.append(("Module Import", True, "Main module imports successfully"))
        except Exception as e:
            validation_results.append(("Module Import", False, f"Failed to import main module: {e}"))
        ...
        try:
            ...
            validation_results.append(("Configuration", True, "Configuration files are readable"))
        except Exception as e:
            validation_results.append(("Configuration", False, f"Configuration validation failed: {e}"))
        ...
        try:
            ...
            if broken_links:
                validation_results.append(("File System", False, f"Found {len(broken_links)} broken symlinks"))
            else:
                validation_results.append(("File System", True, "No broken symlinks found"))
                
        except Exception as e:
            validation_results.append(("File System", False, f"File system check failed: {e}"))
            ...
            if context.backup_id:
                ...
                try:
                    ...
                    if choice == "1" or choice.upper() == "AUTO":
                        ...
                        return GateResult.PASS, "Validation failed - automatic rollback completed"
                    elif choice == "2" or choice.upper() == "MANUAL":
                        safe_print(f"Manual rollback available with backup ID: {context.backup_id}")
                        return GateResult.FAIL, f"Validation failed - manual rollback required (backup: {context.backup_id})"
                    else:
                        self.log(context, "User chose to continue despite validation failures")
                        return GateResult.PASS, "Validation failed - user chose to continue"
                ...
                return GateResult.FAIL, f"Validation failed with {len(failures)} failures - no backup available"
...
class CleanupSafetyGateFramework:
    ...
    def execute_cleanup_operation(self, operation: CleanupOperation) -> Tuple[bool, str]:
        """
    ...
    def rollback_operation(self, backup_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            context = SafetyGateContext(operation=CleanupOperation("rollback", [], "Manual rollback"))
            context.backup_id = backup_id
...
def safe_cleanup_operation(root: Path, operation_type: str, targets: List[Path], 
