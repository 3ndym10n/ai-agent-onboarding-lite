...
class AIAgentIASWrapper:
    ...
    def validate_conversation_boundaries(self, user_input: str) -> Tuple[bool, str]:
        ...
        for locked in self.guardrails.LOCKED_DECISIONS:
            if locked.replace("_", " ") in input_lower:
                return False, f"Cannot discuss changing {locked} - it's a locked decision"
    ...
    def process_user_response(self, user_input: str, alignment_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(word in input_lower for word in ["yes", "correct", "right", "proceed", "continue", "good"]):
            return {
                "action": "confirmed",
        ...
        # Check for rejection
        elif any(word in input_lower for word in ["no", "wrong", "incorrect", "stop", "cancel"]):
            return {
                "action": "rejected",
        ...
        elif any(word in input_lower for word in ["clarify", "explain", "what", "how", "why"]):
            return {
        ...
        elif any(word in input_lower for word in ["actually", "but", "however", "instead", "rather"]):
            return {
    ...
    def execute_command_with_flags(self, command: str, flags: List[str] = None) -> Dict[str, Any]:
