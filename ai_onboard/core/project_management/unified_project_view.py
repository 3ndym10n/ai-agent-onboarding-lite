...
class UnifiedProjectView:
    ...
    def _load_charter(self) -> Dict[str, Any]:
        ...
        return utils.read_json(self.charter_file, default={})  # type: ignore[no-any-return]

    def _load_plan(self) -> Dict[str, Any]:
        """Load project plan."""
        return utils.read_json(self.plan_file, default={})  # type: ignore[no-any-return]

    def _load_wbs(self) -> Dict[str, Any]:
        """Load WBS data."""
        return utils.read_json(self.wbs_file, default={})  # type: ignore[no-any-return]
    ...
    def _get_next_tasks(
        self, wbs: Dict[str, Any], count: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    def get_status_summary(self) -> str:
        ...
        summary.append(
            f"   {dashboard.progress_percentage:.0f}% complete | {dashboard.current_phase}"
        )

        if dashboard.in_progress_task:
            summary.append(
                f"   Current: {dashboard.in_progress_task.get('name', 'Unknown')}"
            )

        if dashboard.next_tasks:
            summary.append(f"   Next: {dashboard.next_tasks[0].get('name', 'Unknown')}")
