...
class AIOnboardAPIServer:
    ...
    def _add_routes(self, app: FastAPI):
        ...
        async def register_agent(request: AgentRegistrationRequest):
            ...
            try:
                ...
                capabilities = [capability_map.get(cap) for cap in request.capabilities if cap in capability_map]
                ...
                agent_profile = AgentProfile(
                    ...
                    collaboration_mode=mode_map.get(request.collaboration_mode, CollaborationMode.COLLABORATIVE),
                    safety_level=safety_map.get(request.safety_level, SafetyLevel.MEDIUM),
        ...
        @app.post("/api/v1/agents/session/create", response_model=SessionCreateResponse)
        ...
        @app.post("/api/v1/agents/session/{session_id}/command", response_model=CommandExecutionResponse)
        ...
        async def translate_natural_language(request: NaturalLanguageRequest):
            ...
            try:
                result = self.cursor_integration.translate_natural_language_command(request.text)
    ...
    async def _run_analysis(self, operation_id: str, allow_exec: bool, target_path: Optional[str]):
    ...
    async def _run_charter_creation(self, operation_id: str, request: CharterRequest):
        ...
        try:
            ...
            result = charter.create_charter(self.root, charter_data, interactive=request.interactive)
    ...
    async def _run_validation(self, operation_id: str, request: ValidationRequest):
        ...
        try:
            ...
            # Run validation
            result = validation_runtime.validate_project(self.root, generate_report=request.generate_report)
    ...
    async def _broadcast_progress_update(
        self, operation_id: str, operation_type: str, progress: float,
        current_step: str, completed_steps: int, total_steps: int
    ):
...
def get_api_server(root: Path, host: str = "127.0.0.1", port: int = 8080) -> AIOnboardAPIServer:
