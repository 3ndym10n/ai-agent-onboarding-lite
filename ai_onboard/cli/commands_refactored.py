...
from .commands_enhanced_vision import add_enhanced_vision_parser, handle_enhanced_vision_commands
from .commands_ai_agent_collaboration import add_ai_agent_collaboration_parser, handle_ai_agent_collaboration_commands
...
def main(argv=None):
    ...
    if args.cmd == "interrogate":
        with error_monitor.monitor_command_execution("interrogate", "foreground", "cli_session"):
            if handle_interrogate_commands(args, root):
                return

    # Handle prompt commands with error monitoring
    if args.cmd == "prompt":
        with error_monitor.monitor_command_execution("prompt", "foreground", "cli_session"):
            if handle_prompt_commands(args, root):
                return

    # Handle AI agent commands with error monitoring
    if args.cmd == "ai-agent":
        with error_monitor.monitor_command_execution("ai-agent", "foreground", "cli_session"):
            if handle_ai_agent_commands(args, root):
                return

    # Handle AAOL commands with error monitoring
    if args.cmd == "aaol":
        with error_monitor.monitor_command_execution("aaol", "foreground", "cli_session"):
            if handle_aaol_commands(args, root):
                return

    # Handle enhanced vision commands with error monitoring
    if args.cmd == "enhanced-vision":
        with error_monitor.monitor_command_execution("enhanced-vision", "foreground", "cli_session"):
    ...
    if args.cmd == "ai-collaboration":
        with error_monitor.monitor_command_execution("ai-collaboration", "foreground", "cli_session"):
