...
def add_dependency_check_commands(subparsers):
    ...
    dep_subparsers = dep_parser.add_subparsers(dest="dep_action", help="Dependency check actions")
    
    # Check specific files
    check_parser = dep_subparsers.add_parser("check", help="Check dependencies for specific files")
    check_parser.add_argument("files", nargs="+", help="Files to check for dependencies")
    check_parser.add_argument("--detailed", action="store_true", help="Show detailed dependency information")
    check_parser.add_argument("--report", action="store_true", help="Generate comprehensive report")
    
    # Scan for all dependencies in project
    scan_parser = dep_subparsers.add_parser("scan", help="Scan entire project for dependency relationships")
    scan_parser.add_argument("--pattern", help="File pattern to scan (e.g., '*.py', '*.md')")
    scan_parser.add_argument("--output", help="Save results to file")
    
    # Validate cleanup safety
    validate_parser = dep_subparsers.add_parser("validate-cleanup", help="Validate that cleanup operations are safe")
    validate_parser.add_argument("--dry-run", action="store_true", help="Show what would be checked")
...
def handle_dependency_check_commands(args, root: Path):
    ...
    if not hasattr(args, 'dep_action') or not args.dep_action:
        print_status("No dependency check action specified. Use --help for options.", "error")
        return
...
def _handle_check_files(args, root: Path):
        ...
        if unsafe_files > 0:
            ...
            for result in results:
                if not result.is_safe_to_delete:
                    safe_print(f"    ⚠️ {result.target_file.name} ({len(result.dependencies)} deps)")
    ...
    if unsafe_results:
        ...
        if fix_plan['manual_review_needed']:
            safe_print(f"   Manual review needed: {len(fix_plan['manual_review_needed'])}")
...
def _handle_scan_project(args, root: Path):
    ...
    if args.output:
        ...
        output_data = {
            ...
            "top_dependencies": dict(sorted(dependency_map.items(), key=lambda x: x[1], reverse=True)[:20])
        }
...
def _handle_validate_cleanup(args, root: Path):
    ...
    try:
        ...
        print_content(f"Found {len(non_critical_files)} files that would be deleted", "stats")
        ...
        if is_safe:
            print_status("✅ Cleanup validation passed - all files are safe to delete", "success")
        else:
            print_status("❌ Cleanup validation failed - some files have dependencies", "error")
            safe_print("\nRecommendation: Fix dependencies before running cleanup")
            safe_print("Or use 'ai_onboard dependency-check check <file>' for detailed analysis")
...
def check_file_dependencies(root: Path, files: List[str], detailed: bool = False) -> bool:
    """
