...
class UXMiddleware:
    ...
    def _show_pending_interventions(self, user_id: str):
        ...
        try:
            ...
            for intervention in high_priority[:1]:  # Show max 1 high priority
                ...
                if message:
                    print()
                    print(ui_system.format_output("🔔 Helpful Assistance", "info", user_id))
                    print(message)
                    print()
    ...
    def _handle_command_error(self, error: Exception, command: str, user_id: str):
        ...
        try:
            ...
            intervention = ux_system.error_handler.handle_error(error, error_context, user_id)
            
            # Show error recovery guidance
            print()
            print(ui_system.format_output("🚨 Error Recovery Assistance", "error", user_id))
            message = ux_system.deliver_intervention(intervention.intervention_id)
            if message:
                print(message)
            print()
    ...
    def _record_command_execution(self, command: str, user_id: str, success: bool, 
                                error_details: Optional[str], duration_ms: float):
        """Record command execution event."""
        try:
            ux_system, ui_system, capability_tracker, status = self._get_systems(user_id)
    ...
    def _check_post_execution_interventions(self, command: str, user_id: str):
        ...
        try:
            ...
            interventions = ux_system.get_pending_interventions(user_id)
            workflow_interventions = [i for i in interventions 
                                    if i.intervention_type.value == "workflow_guidance" 
                                    and i.priority <= 2]
            ...
            if workflow_interventions:
                ...
                if message:
                    print()
                    print(ui_system.format_output("💡 Workflow Suggestion", "secondary", user_id))
                    print(message)
                    print()
            ...
            satisfaction_interventions = [i for i in interventions 
                                        if i.intervention_type.value == "satisfaction_check"]
            
            if satisfaction_interventions and len(interventions) <= 1:  # Only if no other interventions
    ...
    def show_welcome_message(self, user_id: str = "default"):
        ...
        try:
            ...
            if total_usage == 0:
                print()
                print(ui_system.format_output("🎉 Welcome to AI Onboard!", "primary", user_id))
            ...
            # Show returning user message
                ...
                print()
                print(status.info(f"Welcome back! You've used {total_usage} commands so far."))
    ...
    def show_completion_celebration(self, user_id: str = "default"):
        ...
        try:
            ...
            if total_usage in milestones:
                print()
                print(status.completed(f"🎉 Milestone reached: {total_usage} commands executed!"))
                
                if total_usage == 5:
                    print("You're getting familiar with AI Onboard. Try 'help --tutorial' for advanced features.")
                elif total_usage == 10:
                    print("Great progress! Explore 'kaizen' for continuous improvement features.")
                elif total_usage == 25:
                    print("You're becoming an AI Onboard expert! Check out 'opt-experiments' for advanced optimization.")
                elif total_usage >= 50:
                    print("Outstanding! You're a power user. Consider 'ux feedback --score 5' to share your experience.")
