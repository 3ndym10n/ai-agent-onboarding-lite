...
def add_optimization_experiment_commands(subparsers):
    ...
    design_parser = exp_sub.add_parser("design", help="Design a new optimization experiment")
    design_parser.add_argument("--name", required=True, help="Experiment name")
    design_parser.add_argument("--description", required=True, help="Experiment description")
    design_parser.add_argument("--type", choices=["ab_test", "multivariate", "sequential", "canary"], 
                              default="ab_test", help="Experiment type")
    design_parser.add_argument("--primary-metric", required=True, help="Primary metric to optimize")
    design_parser.add_argument("--sample-size", type=int, default=100, help="Sample size per condition")
    design_parser.add_argument("--min-runtime", type=int, default=1, help="Minimum runtime in hours")
    design_parser.add_argument("--max-runtime", type=int, default=24, help="Maximum runtime in hours")
    design_parser.add_argument("--confidence", type=float, default=0.95, help="Confidence level")
    design_parser.add_argument("--min-effect", type=float, default=0.05, help="Minimum detectable effect")
    ...
    stop_parser.add_argument("--reason", default="manual_stop", help="Reason for stopping")
    
    # Status command
    status_parser = exp_sub.add_parser("status", help="Show experiment status")
    status_parser.add_argument("experiment_id", nargs="?", help="Specific experiment ID (optional)")
    status_parser.add_argument("--detailed", action="store_true", help="Show detailed information")
    
    # Record measurement command
    record_parser = exp_sub.add_parser("record", help="Record a measurement for running experiment")
    ...
    results_parser.add_argument("--detailed", action="store_true", help="Show detailed analysis")
    
    # List experiments command
    list_parser = exp_sub.add_parser("list", help="List all experiments")
    list_parser.add_argument("--status", choices=["running", "completed", "all"], default="all", help="Filter by status")
    ...
    # Templates command
    templates_parser = exp_sub.add_parser("templates", help="Experiment design templates")
    templates_sub = templates_parser.add_subparsers(dest="template_action", required=True)
    
    # List templates
    templates_sub.add_parser("list", help="List available experiment templates")
    
    # Create from template
    create_template_parser = templates_sub.add_parser("create", help="Create experiment from template")
...
def handle_optimization_experiment_commands(args: argparse.Namespace, root: Path) -> None:
...
def _handle_design_experiment(args: argparse.Namespace, framework) -> None:
    ...
    metrics = [
        ExperimentMetric(
            ...
            higher_is_better=False if "time" in args.primary_metric.lower() or "error" in args.primary_metric.lower() else True
        ),
        ExperimentMetric(
            name="throughput",
    ...
    print(f"Runtime: {design.minimum_runtime_hours}-{design.maximum_runtime_hours} hours")
    
    print(f"\n📋 Conditions:")
    for condition in design.conditions:
        print(f"   • {condition.name} ({condition.condition_id})")
        print(f"     Risk: {condition.risk_level}/5, Expected: {condition.expected_impact}%")
...
def _handle_start_experiment(args: argparse.Namespace, framework) -> None:
    ...
    if success:
        ...
        print(f"📝 Record data: opt-experiments record {args.experiment_id} <condition> <metric> <value>")
    else:
...
def _handle_status(args: argparse.Namespace, framework) -> None:
    ...
    if args.experiment_id:
        ...
        unstarted = {exp_id: name for exp_id, name in designs.items() 
    ...
    if args.detailed:
        ...
        if 'condition_results' in analysis:
            ...
            for condition, stats in analysis['condition_results'].items():
                print(f"   {condition}: mean={stats['mean']:.3f}, std={stats['std']:.3f}, n={stats['count']}")
...
def _handle_analytics_commands(args: argparse.Namespace, framework) -> None:
    ...
    if args.analytics_action == "success":
        ...
        successful = len([exp for exp in completed.values() if exp.get('outcome') in ['success']])
        harmful = len([exp for exp in completed.values() if exp.get('outcome') == 'harmful'])
        inconclusive = len([exp for exp in completed.values() if exp.get('outcome') == 'inconclusive'])
        ...
        print(f"Inconclusive: {inconclusive} ({inconclusive/total_experiments*100:.1f}%)")
        
        # Average improvement
        improvements = [exp.get('improvement_percentage', 0) for exp in completed.values()]
        if improvements:
...
def _handle_template_commands(args: argparse.Namespace, framework) -> None:
    ...
    if args.template_action == "list":
        ...
        print(f"\n🚀 Usage:")
        print(f"   opt-experiments templates create <template_id> --name 'My Experiment'")
        ...
        print("💡 For now, use: opt-experiments design --name 'My Experiment' --description '...'")
