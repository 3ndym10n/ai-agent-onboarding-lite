...
def add_performance_trend_commands(subparsers):
    ...
    subcommands = parser.add_subparsers(
        dest="trend_cmd", help="Performance trend commands"
    )
    ...
    forecast_parser.add_argument(
        "--confidence", action="store_true", help="Include confidence intervals"
    )
...
def handle_performance_trend_commands(args: argparse.Namespace, root: Path) -> None:
    ...
    if not hasattr(args, "trend_cmd") or args.trend_cmd is None:
        print(
            "❌ No performance trend command specified. Use --help for available commands."
        )
        return
...
def _handle_analyze_trends(args: argparse.Namespace, root: Path) -> None:
    ...
    try:
        ...
        if args.severity:
            ...
            trends = [
                t for t in trends if severity_order.get(t.severity, 0) >= min_level
            ]
...
def _handle_generate_forecast(args: argparse.Namespace, root: Path) -> None:
    ...
    try:
        ...
        if not forecast:
            print(
                f"⚠️ Unable to generate forecast for {args.metric} - insufficient data or configuration disabled"
            )
            return
...
def _handle_generate_insights(args: argparse.Namespace, root: Path) -> None:
    ...
    try:
        ...
        if args.priority:
            ...
            insights = [
                i for i in insights if priority_order.get(i.priority, 0) >= min_level
            ]
...
def _display_trends_table(trends):
    ...
    print("-" * 80)
    print(
        f"{'Metric':<20} {'Direction':<12} {'Severity':<10} {'Change%':<8} {'Confidence':<10} {'Points':<8}"
    )
    print("-" * 80)
    ...
    for trend in trends:
        ...
        print(
            f"{trend.metric_name:<20} {direction_icon} {trend.direction.value:<10} "
...
def _display_trends_summary(trends):
    ...
    for trend in trends:
        direction_counts[trend.direction.value] = (
            direction_counts.get(trend.direction.value, 0) + 1
        )
        severity_counts[trend.severity.value] = (
            severity_counts.get(trend.severity.value, 0) + 1
        )
...
def _display_anomalies_table(anomalies):
    ...
    print("-" * 90)
    print(
        f"{'Metric':<20} {'Type':<10} {'Severity':<10} {'Value':<10} {'Expected':<10} {'Deviation':<10} {'Time':<15}"
    )
    print("-" * 90)
...
def _display_insights_table(insights):
    ...
    print("-" * 80)
    print(
        f"{'Title':<30} {'Category':<12} {'Priority':<10} {'Impact':<10} {'Metrics':<15}"
    )
    print("-" * 80)
    ...
    for insight in insights:
        ...
        print(
            f"{insight.title[:28]:<30} {insight.category:<12} "
...
def _display_compact_dashboard(trends, anomalies, insights):
    ...
    trend_status = (
        "🔴"
        if any(t.severity == TrendSeverity.CRITICAL for t in trends)
        else "🟡" if any(t.severity == TrendSeverity.HIGH for t in trends) else "🟢"
    )

    anomaly_status = (
        "🔴"
        if any(a.severity == TrendSeverity.CRITICAL for a in anomalies)
        else "🟡" if any(a.severity == TrendSeverity.HIGH for a in anomalies) else "🟢"
    )

    insight_status = (
        "🔴"
        if any(i.priority == "critical" for i in insights)
        else "🟡" if any(i.priority == "high" for i in insights) else "🟢"
    )
...
def _display_history_table(historical_trends):
    ...
    print("-" * 70)
    print(
        f"{'Date':<12} {'Metric':<20} {'Direction':<12} {'Severity':<10} {'Change%':<8}"
    )
    print("-" * 70)
    ...
    for trend_data in sorted(
        ...
        print(
            f"{date_str:<12} {trend_data['metric_name']:<20} "
...
def _display_history_timeline(historical_trends):
    ...
    for date_key in sorted(by_date.keys(), reverse=True):
        ...
        for trend_data in by_date[date_key]:
            ...
            print(
                f"   {time_str} - {trend_data['metric_name']}: {trend_data['direction']} ({trend_data['severity']})"
            )
