...
from ..core.unicode_utils import safe_print, print_status, print_content, print_activity
...
def add_cleanup_safety_commands(subparsers):
    ...
    safety_subparsers = safety_parser.add_subparsers(dest="safety_action", help="Safety actions")
    
    # Test safety gates
    test_parser = safety_subparsers.add_parser("test", help="Test safety gates with a file")
    test_parser.add_argument("files", nargs="+", help="Files to test safety gates with")
    test_parser.add_argument("--operation", choices=["delete", "move", "modify"], 
                           default="delete", help="Type of operation to test")
    test_parser.add_argument("--dry-run", action="store_true", 
                           help="Show what would happen without executing")
    
    # List backups
    list_parser = safety_subparsers.add_parser("list-backups", help="List available backups")
    list_parser.add_argument("--limit", type=int, default=10, help="Maximum number of backups to show")
    
    # Rollback operation
    rollback_parser = safety_subparsers.add_parser("rollback", help="Rollback a previous operation")
    ...
    # Clean old backups
    clean_parser = safety_subparsers.add_parser("clean-backups", help="Clean old backup files")
    ...
    config_parser = safety_subparsers.add_parser("config", help="Configure safety gate settings")
    config_parser.add_argument("--strict-mode", choices=["on", "off"], 
                             help="Enable/disable strict safety mode")
    config_parser.add_argument("--auto-rollback", choices=["on", "off"],
                             help="Enable/disable automatic rollback on failure")
    config_parser.add_argument("--show", action="store_true", help="Show current configuration")
    
    # Status command
    status_parser = safety_subparsers.add_parser("status", help="Show safety gate status")
    ...
    for i, backup in enumerate(backups[:args.limit]):
        ...
        if backup['operation']['targets']:
            ...
            if len(backup['operation']['targets']) > 3:
                safe_print(f"     ... and {len(backup['operation']['targets']) - 3} more")
...
def _handle_clean_backups(args, root: Path):
    ...
    for backup_dir in backups_dir.iterdir():
        if backup_dir.is_dir():
            try:
                ...
                timestamp_str = backup_dir.name.split('_')[2] + '_' + backup_dir.name.split('_')[3]
    ...
    if args.dry_run:
        ...
        for backup_dir, backup_date, size in old_backups:
            ...
            safe_print(f"  - {backup_dir.name} ({age_days} days old, {size/(1024*1024):.1f} MB)")
    ...
    print_status(f"Cleanup completed: {deleted_count} backups deleted, {deleted_size/(1024*1024):.1f} MB freed", "success")
...
def _handle_config(args, root: Path):
    ...
    if args.strict_mode:
        ...
        changed = True
        print_status(f"Strict mode: {'enabled' if config['strict_mode'] else 'disabled'}", "info")
    
    if args.auto_rollback:
        config["auto_rollback_on_failure"] = (args.auto_rollback == "on")
        changed = True
        print_status(f"Auto rollback: {'enabled' if config['auto_rollback_on_failure'] else 'disabled'}", "info")
    ...
    if args.show or not changed:
        ...
        safe_print("=" * 40)
        safe_print(f"Strict mode: {'✅ Enabled' if config['strict_mode'] else '❌ Disabled'}")
        safe_print(f"Auto rollback: {'✅ Enabled' if config['auto_rollback_on_failure'] else '❌ Disabled'}")
        safe_print(f"Backup retention: {config['backup_retention_days']} days")
        safe_print(f"Medium risk confirmation: {'✅ Required' if config['require_confirmation_for_medium_risk'] else '❌ Not required'}")
