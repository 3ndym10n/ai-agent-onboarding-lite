...
def add_metrics_commands(subparsers):
    ...
    metrics_parser = subparsers.add_parser("unified-metrics", help="Unified metrics collection and analysis")
    ...
    query_parser.add_argument("--metric", help="Metric name or pattern (supports wildcards)")
    query_parser.add_argument("--source", choices=[s.value for s in MetricSource], help="Metric source")
    query_parser.add_argument("--category", choices=[c.value for c in MetricCategory], help="Metric category")
    ...
    query_parser.add_argument("--agg", choices=["sum", "avg", "min", "max", "count"], help="Aggregation function")
    query_parser.add_argument("--format", choices=["json", "table", "csv"], default="table", help="Output format")
    query_parser.add_argument("--dimension", action="append", help="Filter by dimension (key=value)")
    ...
    trend_parser.add_argument("--days", type=int, default=7, help="Number of days to analyze")
    trend_parser.add_argument("--interval", default="1h", help="Time interval for grouping")
    trend_parser.add_argument("--format", choices=["json", "table", "chart"], default="table", help="Output format")
    
    # Stats command
    stats_parser = metrics_sub.add_parser("stats", help="Show collection statistics")
    stats_parser.add_argument("--format", choices=["json", "table"], default="table", help="Output format")
    ...
    alert_list_parser.add_argument("--severity", choices=["low", "medium", "high", "critical"], help="Filter by severity")
    ...
    alert_create_parser.add_argument("--condition", required=True, help="Condition (e.g., '> 90')")
    alert_create_parser.add_argument("--severity", choices=["low", "medium", "high", "critical"], default="medium")
    
    # Report command
    report_parser = metrics_sub.add_parser("report", help="Generate metrics reports")
    report_parser.add_argument("--type", choices=["performance", "usage", "health", "custom"], default="performance")
    report_parser.add_argument("--period", default="24h", help="Report period")
    report_parser.add_argument("--format", choices=["json", "html", "csv"], default="json")
    ...
    export_parser.add_argument("--format", choices=["json", "csv", "jsonl"], default="csv")
...
def _handle_metrics_alert(args: argparse.Namespace, collector) -> None:
    ...
    if args.alert_action == "list":
        ...
        print(f"✅ Alert rule created: {args.metric} {args.condition} (severity: {args.severity})")
...
def _handle_metrics_export(args: argparse.Namespace, collector) -> None:
    ...
    if args.format == "json":
        ...
        with open(args.output, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["id", "name", "value", "source", "category", "timestamp", "unit"])
            
            for m in result.metrics:
                writer.writerow([
                    m.id, m.name, m.value, m.source.value, m.category.value,
                    m.timestamp.isoformat(), m.unit or ""
                ])
...
def _output_csv_result(result) -> None:
    ...
    for m in result.metrics:
        writer.writerow([
            m.name, m.value, m.source.value, m.category.value,
            m.timestamp.isoformat(), m.unit or ""
        ])
...
def _output_table_result(result, aggregation) -> None:
    ...
    print(f"📊 Query Results ({result.total_count} metrics, {result.query_time_ms:.1f}ms)")
    ...
    for m in result.metrics[:20]:  # Limit display
        ...
        print(f"{m.name:<25} {m.value:<12}{unit_str} {m.source.value:<12} {m.category.value:<12} {timestamp_str:<16}")
...
def _display_trend_table(intervals: Dict, metric_name: str) -> None:
    ...
    for time_key in sorted(intervals.keys()):
        ...
        print(f"{time_key:<16} {data['count']:<8} {data['avg']:<10.2f} {data['min']:<10.2f} {data['max']:<10.2f}")
...
def _generate_usage_report(collector, start_time, end_time) -> Dict:
    ...
    return {
        ...
        "unique_users": len(set(m.dimensions.get("user_id", "unknown") for m in result.metrics))
    }
