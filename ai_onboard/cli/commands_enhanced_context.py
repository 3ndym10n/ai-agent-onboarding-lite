...
def add_enhanced_context_commands(subparsers):
    ...
    enhance_parser = context_sub.add_parser(
        "enhance", 
        help="Enhance a session with cross-session context and memories"
    )
    enhance_parser.add_argument("--session-id", required=True, help="Session ID to enhance")
    ...
    # Create memory
    create_memory_parser = memory_sub.add_parser("create", help="Create a new context memory")
    ...
    create_memory_parser.add_argument("--facts", nargs="+", required=True, help="Key facts")
    create_memory_parser.add_argument("--importance", choices=["low", "normal", "high", "critical"], 
                                    default="normal", help="Memory importance level")
    ...
    list_memory_parser.add_argument("--limit", type=int, default=20, help="Limit results")
    
    # Share context command
    share_parser = context_sub.add_parser("share", help="Share context with another AI agent")
    ...
    share_parser.add_argument("--context-types", nargs="+", 
                            choices=["conversation_history", "user_preferences", "project_context", "error_patterns"],
                            default=["conversation_history", "project_context"],
                            help="Types of context to share")
    
    # Continuity command
    continuity_parser = context_sub.add_parser("continuity", help="Get context continuity summary")
    ...
    # Find patterns
    patterns_parser = graph_sub.add_parser("patterns", help="Find conversation patterns")
    ...
    # Analytics command
    analytics_parser = context_sub.add_parser("analytics", help="Context analytics and insights")
    analytics_sub = analytics_parser.add_subparsers(dest="analytics_action", required=True)
    
    # User insights
    insights_parser = analytics_sub.add_parser("insights", help="Get user behavior insights")
    ...
    cleanup_parser.add_argument("--days", type=int, default=30, help="Keep data newer than N days")
...
def _handle_enhance_session(args: argparse.Namespace, context_manager) -> None:
    ...
    print(f"Expertise Areas: {', '.join(insights['expertise_areas']) if insights['expertise_areas'] else 'None identified'}")
    
    if insights["preferred_commands"]:
        print(f"Top Commands: {', '.join(list(insights['preferred_commands'].keys())[:5])}")
    ...
    if args.analytics_action == "insights":
        ...
        if total_memories > 0:
            ...
            for memory in context_manager.memories.values():
                importance_counts[memory.importance_level] = importance_counts.get(memory.importance_level, 0) + 1
...
def _handle_cleanup(args: argparse.Namespace, context_manager) -> None:
    ...
    # Find old memories
    old_memories = [
        memory for memory in context_manager.memories.values()
        if memory.created_at < cutoff_time
    ]
    
    # Find old cross-session contexts
    old_contexts = [
        context for context in context_manager.cross_session_contexts.values()
