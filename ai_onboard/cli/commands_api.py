...
def add_api_commands(subparsers):
    ...
    test_parser.add_argument("--url", default="http://127.0.0.1:8080", help="API server URL")
    ...
    docs_parser.add_argument("--url", default="http://127.0.0.1:8080", help="API server URL")
...
def _handle_api_start(args: argparse.Namespace, root: Path) -> None:
    ...
    try:
        ...
        print(f"   Project Status: http://{args.host}:{args.port}/api/v1/project/status")
...
def _handle_api_status(args: argparse.Namespace, root: Path) -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                status_response = requests.get(f"{url}/api/v1/project/status", timeout=5)
                if status_response.status_code == 200:
                    project_data = status_response.json()
                    print(f"   Project Progress: {project_data.get('overall_progress', 0):.1f}%")
