...
def add_capability_tracking_commands(subparsers):
    ...
    cap_parser = subparsers.add_parser(
        "capability-tracking", 
        help="System capability usage tracking and analytics"
    )
    ...
    capability_parser = metrics_sub.add_parser("capability", help="View specific capability metrics")
    capability_parser.add_argument("capability_name", help="Name of the capability")
    
    # View category metrics
    category_parser = metrics_sub.add_parser("category", help="View metrics by category")
    category_parser.add_argument("category", choices=[c.value for c in CapabilityCategory], 
    ...
    trends_parser.add_argument("--category", choices=[c.value for c in CapabilityCategory],
                              help="Category to analyze")
    
    # User profiles
    profile_parser = cap_sub.add_parser("profile", help="User capability profiles")
    profile_parser.add_argument("--user-id", default="default", help="User ID to analyze")
    profile_parser.add_argument("--detailed", action="store_true", help="Show detailed profile")
    
    # Reports
    report_parser = cap_sub.add_parser("report", help="Generate capability usage reports")
    ...
    # Analytics
    analytics_parser = cap_sub.add_parser("analytics", help="Advanced capability analytics")
    ...
    # Configuration
    config_parser = cap_sub.add_parser("config", help="Capability tracking configuration")
    ...
    update_parser.add_argument("--tracking-enabled", type=bool, help="Enable/disable tracking")
    update_parser.add_argument("--detailed-tracking", type=bool, help="Enable detailed tracking")
    update_parser.add_argument("--retention-days", type=int, help="Data retention period")
    ...
    if args.metrics_action == "all":
        ...
        for name, metrics in sorted(tracker.capability_metrics.items()):
            rows.append([
                ...
                f"{metrics.avg_duration_ms:.0f}ms" if metrics.avg_duration_ms > 0 else "N/A"
            ])
        ...
        avg_success_rate = sum(m.success_rate for m in tracker.capability_metrics.values()) / len(tracker.capability_metrics)
        ...
        if metrics.common_contexts:
            ...
            for context, count in sorted(metrics.common_contexts.items(), 
                                       key=lambda x: x[1], reverse=True)[:5]:
                percentage = (count / metrics.total_uses) * 100
                print(f"   • {context.replace('_', ' ').title()}: {count} ({percentage:.1f}%)")
        
        if metrics.common_patterns:
            print(f"\n📋 Usage Patterns:")
            for pattern, count in sorted(metrics.common_patterns.items(), 
                                       key=lambda x: x[1], reverse=True)[:5]:
                percentage = (count / metrics.total_uses) * 100
        ...
        if metrics.peak_usage_hours:
            print(f"\n🕐 Peak Usage Hours:")
            sorted_hours = sorted(metrics.peak_usage_hours.items(), 
                                key=lambda x: x[1], reverse=True)[:5]
        ...
        for name, metrics in sorted(category_metrics.items(), 
                                  key=lambda x: x[1].total_uses, reverse=True):
        ...
        avg_success_rate = sum(m.success_rate for m in category_metrics.values()) / len(category_metrics)
...
def _handle_trends_commands(args: argparse.Namespace, tracker, root: Path) -> None:
    ...
    if args.capability:
        ...
        if recent_usage:
            ...
            daily_data = {date.split('-')[-1]: recent_usage[date] for date in sorted_dates}
        ...
        print(f"📊 Trends for {category.value.replace('_', ' ').title()} (Last {args.days} days)")
        print("-" * 60)
        ...
        if trending_caps:
            ...
            sorted_trending = sorted(trending_caps.items(), key=lambda x: x[1], reverse=True)[:10]
        ...
        for name, metrics in tracker.capability_metrics.items():
            ...
            category_usage[category_name] = category_usage.get(category_name, 0) + metrics.total_uses
...
def _handle_profile_commands(args: argparse.Namespace, tracker, root: Path) -> None:
    ...
    if categories and args.detailed:
        chart = create_chart(root, "default")
        print(f"\n📊 Category Preferences:")
        category_display = {k.replace('_', ' ').title(): v for k, v in categories.items()}
...
def _handle_report_commands(args: argparse.Namespace, tracker, root: Path) -> None:
    ...
    # Text format report
    print(f"Report Period: {report.period_start.strftime('%Y-%m-%d')} to {report.period_end.strftime('%Y-%m-%d')}")
    ...
    if report.peak_usage_times:
        ...
        print("-" * 25)
        sorted_hours = sorted(report.peak_usage_times.items(), 
                            key=lambda x: x[1], reverse=True)[:5]
        for hour, count in sorted_hours:
            print(f"   • {hour:02d}:00-{hour:02d}:59: {count} uses")
        print()
...
def _handle_analytics_commands(args: argparse.Namespace, tracker, root: Path) -> None:
    ...
    if args.analytics_action == "performance":
        ...
        if all_patterns:
            ...
            chart = create_chart(root, "default")
            pattern_display = {k.replace('_', ' ').title(): v for k, v in all_patterns.items()}
            print(chart.bar_chart(pattern_display, max_width=40))
        
        if all_contexts:
            print(f"\n🔍 System-wide Usage Contexts:")
            context_display = {k.replace('_', ' ').title(): v for k, v in all_contexts.items()}
        ...
        if not any([report.feature_promotion_recommendations, 
...
def _handle_config_commands(args: argparse.Namespace, tracker, root: Path) -> None:
    ...
    if args.config_action == "show":
        ...
        print(f"Performance Tracking: {'✅' if config['performance_tracking'] else '❌'}")
        ...
        if config['excluded_capabilities']:
            print(f"Excluded Capabilities: {', '.join(config['excluded_capabilities'])}")
        else:
