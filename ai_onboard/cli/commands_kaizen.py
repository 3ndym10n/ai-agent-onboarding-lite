...
def add_kaizen_commands(subparsers):
    ...
    # Run cycle command
    run_parser = kaizen_sub.add_parser("run-cycle", help="Run a single Kaizen cycle manually")
    run_parser.add_argument("--verbose", action="store_true", help="Show detailed progress")
    
    # Opportunities command
    opportunities_parser = kaizen_sub.add_parser("opportunities", help="Manage improvement opportunities")
    opp_sub = opportunities_parser.add_subparsers(dest="opp_action", required=True)
    
    # List opportunities
    list_parser = opp_sub.add_parser("list", help="List current improvement opportunities")
    list_parser.add_argument("--category", help="Filter by category")
    list_parser.add_argument("--priority", choices=["low", "medium", "high", "critical"], help="Filter by priority")
    list_parser.add_argument("--status", choices=["identified", "planned", "executing", "completed", "failed", "cancelled"], help="Filter by status")
    ...
    execute_parser.add_argument("--force", action="store_true", help="Force execution even if risky")
    
    # History command
    history_parser = kaizen_sub.add_parser("history", help="Show cycle history")
    history_parser.add_argument("--limit", type=int, default=10, help="Number of cycles to show")
    history_parser.add_argument("--detailed", action="store_true", help="Show detailed cycle information")
    ...
    update_parser.add_argument("--max-concurrent", type=int, help="Max concurrent improvements")
    update_parser.add_argument("--confidence-threshold", type=float, help="Minimum confidence threshold")
    update_parser.add_argument("--auto-execute-low-risk", type=bool, help="Auto-execute low-risk improvements")
    update_parser.add_argument("--enable-category", help="Enable improvement category")
    update_parser.add_argument("--disable-category", help="Disable improvement category")
...
def _handle_start_automation(args: argparse.Namespace, kaizen_engine) -> None:
    ...
    print(f"   Max Concurrent Improvements: {status['config']['max_concurrent_improvements']}")
...
def _handle_status(args: argparse.Namespace, kaizen_engine) -> None:
    ...
    print(f"   Auto-execute Low Risk: {'✅' if config['auto_execute_low_risk'] else '❌'}")
    ...
    # Enabled categories
    enabled_categories = [cat for cat, enabled in config['categories_enabled'].items() if enabled]
    print(f"\n🎯 Enabled Categories:")
    ...
    if recent_cycles:
        ...
        for cycle in recent_cycles:
            print(f"   • {cycle['cycle_id']}: {cycle['overall_impact']} (score: {cycle['success_score']:.2f})")
...
def _handle_run_cycle(args: argparse.Namespace, kaizen_engine) -> None:
    ...
    if args.verbose:
        ...
        for action in cycle.actions_taken:
            ...
            print(f"   {status_icon} {action.get('opportunity', action.get('decision', 'Unknown'))}")
...
def _handle_opportunities_commands(args: argparse.Namespace, kaizen_engine) -> None:
    ...
    if args.opp_action == "list":
        ...
        # Apply filters
        if args.category:
            opportunities = [opp for opp in opportunities if opp.category.value == args.category]
        if args.priority:
            opportunities = [opp for opp in opportunities if opp.priority.value == args.priority]
        if args.status:
            opportunities = [opp for opp in opportunities if opp.status.value == args.status]
...
def _handle_analytics_commands(args: argparse.Namespace, kaizen_engine) -> None:
    ...
    if args.analytics_action == "success":
        ...
        high_impact_cycles = len([c for c in cycles if c["overall_impact"] == "high_impact"])
        
        print(f"Total Cycles: {total_cycles}")
        print(f"Average Success Score: {avg_success_score:.2f}")
        print(f"High Impact Cycles: {high_impact_cycles} ({high_impact_cycles/total_cycles*100:.1f}%)")
        ...
        for opp in opportunities:
            ...
            if category not in category_impacts:
                category_impacts[category] = {"count": 0, "total_impact": 0, "completed": 0}
...
def _handle_config_commands(args: argparse.Namespace, kaizen_engine) -> None:
    ...
    if args.config_action == "show":
        ...
        print(f"Confidence Threshold: {config['min_confidence_threshold']}")
        print(f"Auto-execute Low Risk: {'✅' if config['auto_execute_low_risk'] else '❌'}")
        print(f"Notifications Enabled: {'✅' if config['notification_enabled'] else '❌'}")
