...
def _handle_enhanced_status(args: argparse.Namespace, root: Path) -> None:
    ...
    print(
        f"Phases Completed: {', '.join(status['phases_completed']) if status['phases_completed'] else 'None'}"
    )
    ...
    if status.get("final_quality_report"):
        ...
        print(
            f"Actionable Insights: {report['insight_summary']['actionable_insights']}"
        )
    ...
    for phase, phase_responses in responses.items():
        ...
        for question_id, response_data in phase_responses.items():
            ...
            if metadata:
                ...
                print(
                    f"Technical Terms: {'Yes' if metadata.get('contains_technical_terms') else 'No'}"
                )
                print(
                    f"Business Terms: {'Yes' if metadata.get('contains_business_terms') else 'No'}"
                )
                print(
                    f"User Terms: {'Yes' if metadata.get('contains_user_terms') else 'No'}"
                )
...
def _export_to_markdown(interrogation_data: Dict[str, Any], output_file: Path) -> None:
    ...
    markdown_content.append(
        f"**Project Type:** {interrogation_data.get('project_type', 'Unknown')}"
    )
    markdown_content.append(
...
<body>
    ...
    for phase, phase_responses in responses.items():
        ...
        for question_id, response_data in phase_responses.items():
            ...
            html_content.append(
                f'<p class="confidence">Confidence: {confidence:.1%}</p>'
            )
...
def add_enhanced_vision_parser(subparsers) -> None:
    ...
    enhanced_vision_subparsers = enhanced_vision_parser.add_subparsers(
        dest="enhanced_vision_cmd", help="Enhanced vision commands"
    )
    ...
    enhanced_vision_subparsers.add_parser(
        "status", help="Show enhanced interrogation status"
    )

    # Analyze command
    enhanced_vision_subparsers.add_parser(
        "analyze", help="Analyze responses and generate insights"
    )

    # Export command
    export_parser = enhanced_vision_subparsers.add_parser(
        "export", help="Export interrogation results"
    )
    ...
    templates_parser = enhanced_vision_subparsers.add_parser(
        "templates", help="Manage question templates"
    )
    ...
    create_parser.add_argument("template_name", help="Name for new template")
    create_parser.add_argument("template_file", help="Template definition file")
