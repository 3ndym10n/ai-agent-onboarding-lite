...
class TestPerformanceTrendAnalyzer:
    ...
    def test_initialization(self, temp_root):
        ...
        with patch('ai_onboard.core.performance_trend_analyzer.telemetry'):
            ...
            assert analyzer.config_path == temp_root / ".ai_onboard" / "trend_analysis_config.json"
    ...
    def test_generate_performance_insights(self, trend_analyzer, sample_performance_data):
    ...
    def test_error_handling_invalid_data(self, trend_analyzer):
        ...
        with pytest.raises(ValueError):
            trend_analyzer.analyze_trends(
                "test", 
                [1, 2, 3], 
                [datetime.now(), datetime.now()]  # Different length
            )
    ...
    def test_trend_direction_detection(self, trend_analyzer):
        ...
        trends = trend_analyzer.analyze_trends("increasing", increasing_data, 
                                             [datetime.now() + timedelta(hours=i) for i in range(10)])
        
        # Decreasing trend  
        decreasing_data = list(range(20, 10, -1))
        trends = trend_analyzer.analyze_trends("decreasing", decreasing_data,
                                             [datetime.now() + timedelta(hours=i) for i in range(10)])
        
        # Stable trend
        stable_data = [15] * 10
        trends = trend_analyzer.analyze_trends("stable", stable_data,
                                             [datetime.now() + timedelta(hours=i) for i in range(10)])

    @pytest.mark.parametrize("window_size", [5, 10, 20])
    def test_different_analysis_windows(self, trend_analyzer, sample_performance_data, window_size):
    ...
    def test_anomaly_types_detection(self, trend_analyzer):
        ...
        timestamps = [datetime.now() + timedelta(hours=i) for i in range(len(spike_data))]
        
        anomalies = trend_analyzer.detect_anomalies("spike_test", spike_data, timestamps)
...
class TestPerformanceTrendAnalyzerIntegration:
    ...
    def test_end_to_end_analysis(self, temp_root):
        ...
        with patch('ai_onboard.core.performance_trend_analyzer.telemetry'):
            ...
            response_times = [100 + i * 2 + (i % 3) * 10 for i in range(24)]  # Trend with noise
            
            # Perform analysis
            trends = analyzer.analyze_trends("response_time", response_times, timestamps)
            anomalies = analyzer.detect_anomalies("response_time", response_times, timestamps)
