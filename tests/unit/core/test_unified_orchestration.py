...
class TestUnifiedToolOrchestrator:
    ...
    @patch(
        "ai_onboard.core.orchestration.unified_tool_orchestrator.get_comprehensive_tool_discovery"
    )
    ...
    def test_strategy_routing(self, orchestrator):
        ...
        orchestrator._execute_intelligent_orchestration = Mock(
            return_value=UnifiedOrchestrationResult()
        )
        orchestrator._execute_holistic_orchestration = Mock(
            return_value=UnifiedOrchestrationResult()
        )
        orchestrator._execute_session_orchestration = Mock(
            return_value=UnifiedOrchestrationResult()
        )
        orchestrator._execute_adaptive_orchestration = Mock(
            return_value=UnifiedOrchestrationResult()
        )
...
class TestBackwardCompatibility:
    ...
    @patch(
        "ai_onboard.core.orchestration.orchestration_compatibility.get_unified_tool_orchestrator"
    )
    def test_intelligent_orchestrator_compatibility(self, mock_get_unified, temp_root):
        ...
        mock_unified._analyze_triggers.return_value = [
            {"tool": "test", "confidence": 0.8}
        ]
        ...
        # Test tool execution
        mock_unified._execute_tool_safely.return_value = {
            "executed": True,
            "results": "mocked",
        }
...
class TestErrorHandling:
    ...
    def test_trigger_analysis_populates_matches(self, temp_root):
        orchestrator = UnifiedToolOrchestrator(temp_root)
        context = ToolExecutionContext(
            user_request="Please refactor and cleanup dead code",
            conversation_history=[{"content": "We should refactor the module"}],
            strategy=UnifiedOrchestrationStrategy.TRIGGER_BASED,
        )

        triggers = orchestrator._analyze_triggers(context)
        assert triggers, "Expected trigger recommendations"
        assert any(item["tool"] == "code_quality" for item in triggers)

    def test_execute_tool_safely_runs_code_quality(self, temp_root):
        orchestrator = UnifiedToolOrchestrator(temp_root)
        context = ToolExecutionContext(user_request="Check quality")

        with patch.object(
            orchestrator,
            "_run_code_quality",
            return_value={"results": {"issues": []}, "insights": ["stub"]},
        ):
            result = orchestrator._execute_tool_safely("code_quality_analysis", context)
...
class TestIntegration:
    ...
    @patch(
        "ai_onboard.core.orchestration.unified_tool_orchestrator.PatternRecognitionSystem"
    )
    @patch(
        "ai_onboard.core.orchestration.unified_tool_orchestrator.UserPreferenceLearningSystem"
    )
    @patch("ai_onboard.core.orchestration.unified_tool_orchestrator.get_tool_tracker")
    @patch(
        "ai_onboard.core.orchestration.unified_tool_orchestrator.get_comprehensive_tool_discovery"
