...
def test_error_monitoring():
    ...
    monitor.track_capability_usage("system_test", {"test": "error_monitoring", "success": True})
...
def test_alignment_system():
    ...
    print(f"   - Vision completeness: {alignment_data['components']['vision_completeness']:.2f}")
...
def test_project_plan():
    ...
    print(f"   - Total effort: {sum(t.get('effort_days', 0) for t in plan.get('tasks', []))} days")
