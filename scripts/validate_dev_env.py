...
class DevEnvironmentValidator:
    ...
    def check_system_functionality(self) -> Dict[str, Any]:
        ...
        # Check CLI help
        success, stdout, stderr = self.run_command([sys.executable, "-m", "ai_onboard", "--help"])
        if success:
        ...
        # Check system tests
        success, stdout, stderr = self.run_command([sys.executable, "scripts/test_system.py"])
        if success:
        ...
        # Check unit tests
        success, stdout, stderr = self.run_command([sys.executable, "-m", "pytest", "tests/", "-v"])
        if success:
                ...
                for key, value in results.items():
                    if isinstance(value, dict):
                        for sub_key, sub_value in results[key].items():
                            total_checks += 1
                            if sub_value is True or (isinstance(sub_value, str) and sub_value):
                                passed_checks += 1
                    else:
                        total_checks += 1
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        print(f"Overall Score: {score['passed_checks']}/{score['total_checks']} ({percentage:.1f}%)")
