...
def simulate_ai_agent_conversation():
    ...
    if result.get('needs_update'):
        ...
        print("   💡 Based on your correction, I should focus on real-time data streams.")
    ...
    if updated_alignment.get('decision') == 'proceed':
        ...
        if execution_result["success"]:
            ...
            print("   📊 Your project charter has been updated with real-time orderflow focus.")
        else:
            print(f"   ❌ Analysis failed: {execution_result.get('error')}")
    
    else:
        print(f"   🤔 Still need some alignment. Current confidence: {updated_alignment.get('confidence', 0.0):.2f}")
...
def show_guardrail_examples():
    ...
    print(f"   Min confidence thresholds: {wrapper.guardrails.MIN_CONFIDENCE_THRESHOLDS}")
